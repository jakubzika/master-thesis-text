...
\todo[inline]{This introduction provides a good overview of the chapter structure, but should also include a brief statement about data collection methodology, data limitations, and ethical considerations regarding data usage.}
...
\todo[inline]{Replace the placeholder values X and Y with the actual numbers of charging stations and charging sessions in your dataset. Providing specific details about your dataset size is important for reproducibility and understanding the scope of your analysis.}
...
\begin{figure}[]
    ...
    \todo[inline]{This figure would benefit from additional context in the caption, such as explaining the distribution pattern of charging stations and how it relates to urban density or other relevant factors. Also, ensure that Figure \ref{fig-large:all-chargers} is properly referenced in the appendix.}
\end{figure}
...
\todo[inline]{Explain why hourly granularity was chosen over other potential time intervals (e.g., 15-minute intervals or daily aggregation). Discuss the trade-offs between temporal resolution and model complexity.}
...
The ZSJ dataset provides the following useful data for each area:
...
\todo[inline]{Discuss the potential limitations of using OSM data, such as completeness, accuracy, and currency issues that might affect your analysis. Consider comparing OSM data quality in Prague with other potential data sources.}
...
\todo[inline]{This section would benefit from a discussion of alternative spatial feature engineering approaches that were considered but not implemented, along with the rationale for your final choices. Also, consider adding a summary table that lists all spatial features derived and their expected relationship with charging demand based on literature or domain knowledge.}

\begin{itemize}
    \item[] \textbf{Point in polygon} - Given a point and a set of mutually disjoint areas with features, this method assigns the features of the area containing the point to that point. For example, we assign ZSJ demographic characteristics to charging stations based on the ZSJ polygon in which they are located. This method may be unreliable when the point is near a boundary with other polygons/areas. A more precise solution would be spatial interpolation \sidenote{\url{https://r-spatial.org/book/12-Interpolation.html}}, but for our purposes, the point-in-polygon approach provides sufficient accuracy.
